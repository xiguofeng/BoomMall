...
public class BGAMoocStyleRefreshView extends View {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		TypedArray typedArray = context.obtainStyledAttributes(attrs,
				R.styleable.BGAMoocStyleRefreshView);

		BitmapDrawable originalBitmap = (BitmapDrawable) typedArray
				.getDrawable(R.styleable.BGAMoocStyleRefreshView_mv_originalImg);
		if (originalBitmap == null) {
			throw new RuntimeException(
					BGAMoocStyleRefreshView.class.getSimpleName() + "必须设置原始图片");
		}
		mOriginalBitmap = originalBitmap.getBitmap();

		mUltimateColor = typedArray.getColor(
				R.styleable.BGAMoocStyleRefreshView_mv_ultimateColor,
				Color.rgb(27, 128, 255));
	...
	private void initCanvas() {
		...
		mCanvas = new Canvas();
		mUltimateBitmap = Bitmap.createBitmap(mOriginalBitmapWidth,
				mOriginalBitmapHeight, Config.ARGB_8888);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawBitmap(mUltimateBitmap, getPaddingLeft(), getPaddingTop(),
				null);
		if (mIsRefreshing) {
			invalidate();
		}
	...
	private void drawUltimateBitmap() {
		...
		if (mBezierControlX >= mOriginalBitmapWidth + 1 / 2
				* mOriginalBitmapWidth) {
			mIsBezierControlXIncrease = false;
		} else if (mBezierControlX <= -1 / 2 * mOriginalBitmapWidth) {
			mIsBezierControlXIncrease = true;
		}

		mBezierControlX = mIsBezierControlXIncrease ? mBezierControlX + 10
				: mBezierControlX - 10;
		...
		mBezierPath.cubicTo(mBezierControlX / 2, mWaveY
				- (mBezierControlY - mWaveY),
				(mBezierControlX + mOriginalBitmapWidth) / 2, mBezierControlY,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode == MeasureSpec.EXACTLY) {
			...
			height = mOriginalBitmapHeight + getPaddingTop()
					+ getPaddingBottom();
