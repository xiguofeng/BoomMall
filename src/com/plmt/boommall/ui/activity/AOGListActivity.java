...
import com.plmt.boommall.ui.adapter.AOGAdapter;
...
public class AOGListActivity extends Activity
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case NoticeLogic.AOG_GET_FAIL: {
				break;
			}
			case NoticeLogic.AOG_GET_EXCEPTION: {
				break;
			}
	...
	Handler mCloseHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			int what = msg.what;
			switch (what) {
			case NoticeLogic.AOG_CLOSE_SUC: {
				mProgressDialog.show();
				refreshGoods();
				Toast.makeText(mContext, "删除降价通知成功！", Toast.LENGTH_SHORT).show();
				break;
			}
			case NoticeLogic.AOG_CLOSE_FAIL: {
				if (null != msg.obj) {
			...
			case NoticeLogic.AOG_CLOSE_EXCEPTION: {
				if (null != mProgressDialog && mProgressDialog.isShowing()) {
					mProgressDialog.dismiss();
				}
				break;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.aog_list);
		mContext = AOGListActivity.this;
	...
	private void initView() {
		mBackIv = (ImageView) findViewById(R.id.aog_list_back_iv);
	...
	private void initListView() {
		mNotifyGoodsLv = (ListView) findViewById(R.id.aog_list_goods_lv);

		mAdapter = new AOGAdapter(mContext, mNotifyGoodsList, this);
		...
		mNotifyGoodsLv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					Intent intent = new Intent(AOGListActivity.this, GoodsDetailActivity.class);
	...
	private void refreshGoods() {
		mProgressDialog.show();
		NoticeLogic.getAOG(mContext, mHandler);
	}
		...
		case R.id.list_aog_del_iv: {
			...
			.setPositiveButton(getString(R.string.confirm),
					new OnClickListener() {
						...
						public void onClick(View v) {
							mProgressDialog.show();
							NoticeLogic.closeAOG(mContext, mCloseHandler, mNotifyGoodsList.get(nowPosition).getAlert_id());
						}
					})
